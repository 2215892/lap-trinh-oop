...
public class SwitchMap extends ModelSwitch implements Drawable {
	...
	public void paint(Graphics g) {
		...
		g.fillPolygon(ConversionFunction.polygon(this.position, width, height));
		...
		Point rmad = ConversionFunction.logicToReal(mad);
		Point mbc = getMidPoint(b, c);
		Point rmbc = ConversionFunction.logicToReal(mbc);
		Point mcd = getMidPoint(c, d);
		Point rmcd = ConversionFunction.logicToReal(mcd);
		Point mab = getMidPoint(a, b);
		Point rmab = ConversionFunction.logicToReal(mab);
		...
		if (currentDirection != null) {
			...
			g.fillPolygon(p);
			g.fillPolygon(ConversionFunction.polygon(base, side / 2, side / 2));
	...
	public boolean contains(Point p) {
		...
		Point logicPoint = ConversionFunction.realToLogic(p);
