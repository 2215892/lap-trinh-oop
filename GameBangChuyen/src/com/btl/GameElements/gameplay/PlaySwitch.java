...
public class PlaySwitch extends ModelSwitch implements Drawable {
	...
	if (this.listDirection.size() == 1) { /* Co 1 huong */

	    Direction direction = this.listDirection.get(this.currentDir);
	    ...
	    case DOWN: {
		g.drawImage(DirectionImage.DOWN, coordinate.x, coordinate.y,
			imageSize, imageSize, null);
		break;
	    }
	    case RIGHT: {
		g.drawImage(DirectionImage.RIGHT, coordinate.x, coordinate.y,
			imageSize, imageSize, null);
		break;
	    }
	    case LEFT: {
		g.drawImage(DirectionImage.LEFT, coordinate.x, coordinate.y,
			imageSize, imageSize, null);
		break;
	    }
	    }
	...
	if (logicCoordinate.equals(position))
	    return true;
